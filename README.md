...
| `wire_continued` | Present and true when the response stream closed without finishing while the turn was still generating (a turn that runs tools, such as writing a file, spans several connections); the answer and its files were then read from the page. |
