...
| `wire_continued` | Present and true when the response stream closed without finishing while the turn was still generating (a turn that runs tools, such as writing a file, spans several connections, and so does a turn whose connection dropped and which the site resumed); the answer and its files were then read from the page. |
| `wire_stalled` | Present and true when the page showed the turn finished for `wireStallMs` (default 60 s) while the response stream had never been seen to end, as after a connection lost mid-turn; the answer and its files were then read from the page, and the files named by the turn's download controls. |
| `model_source` | `page` when the model slug was read from the answer's own message on the page (`data-message-model-slug`) because the wire carried none: an answer read from the page after `wire_continued` or `wire_stalled`. `provenance.answered_by` and the model verification use it. |
...
| `provenance.answered_by`, `provenance.sent_as` | Wire, or the page when the wire fell short (see `model_source`). The slug the *server* says answered, and the model the page put in its own request. When a model was requested, `provenance.model.verified` is re-checked against `answered_by`, which outranks any picker label. |
